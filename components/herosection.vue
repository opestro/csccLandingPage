            ...
            <div class="hero-content flex flex-col text-center">
                <div class="max-w-4xl">
                    <h1 class="text-5xl xs:text-4xl font-ntbrick font-bold leading-normal text-[#fe0]"
                        style="font-weight: 400">{{ sheetData.Title }} </h1>
                    <p class="py-6 text-sm  xs:text-xs ">{{ sheetData.Description }}</p>
                    <button @click="write()" v-show="!supportBtn"
                        class="btn bg-[#1c4f82]  hover:text-[#1c2682] hover:bg-[#fe0]  my-5 hover:translate-x-2 hover:translate-y-2
                        tracking-widest shadow-[8px_8px_0_0_rgb(29,33,41)] hover:shadow-none ">{{ sheetData.btnPr }}</button>
                    <div class="text-white" v-if="supportBtn">
                        ...
                        <div class="flex items-center justify-center">
                            ...
                            <input type="text" placeholder="Type ✍️ what you like and share it on your story 🤝"
                                v-model="heroTitle"
...
export default {
    ...
    methods: {
        write() {
            ...
            this.heroTitle = 'Write something you love'
        },
